...
- **Voice-1 (iPhone / Safari):** the mic **records** audio; on stop, audio is sent to `POST /api/concierge/transcribe`. STT order: **`OPENAI_API_KEY`** → **`GROQ_API_KEY`** (recommended where OpenAI is region-restricted) → **`OPENROUTER_API_KEY`** (often OpenAI Whisper upstream; may error in HK / unsupported territories). Safari needs **HTTPS** (or a tunnel). While developing:
