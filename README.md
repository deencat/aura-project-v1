...
- **Voice-1 (iPhone / Safari):** the mic **records** audio; on stop, audio is sent to `POST /api/concierge/transcribe`. STT uses **`OPENAI_API_KEY`** (Whisper) if set, else **`OPENROUTER_API_KEY`** (OpenRouter STT, default `openai/whisper-1`). Safari needs a **secure context** (HTTPS). Two ways while developing:
