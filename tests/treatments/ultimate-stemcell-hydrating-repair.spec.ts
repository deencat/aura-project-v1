...
test.describe('Ultimate Stemcell Hydrating Repair Page', () => {
  ...
  test('loads correctly with TreatmentImage components', async ({ page, baseURL }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /Ultimate Stemcell/i })).toBeVisible();
    ...
    // Verify carousel images are loaded
    ...
    // Check if the benefits section is visible
    await expect(page.getByRole('heading', { name: /Benefits of Ultimate Stemcell Hydrating Repair/i })).toBeVisible();
    
    // Check if the FAQ section is visible
    await expect(page.getByRole('heading', { name: /Frequently Asked Questions/i })).toBeVisible();
    
    // Check if the Book Now button is visible at the bottom
